...
class Settings(BaseSettings):
    ...
    LLM_PROVIDER: str = "anthropic"  # anthropic, openai, ollama, hermes
    LLM_MODEL: str = "claude-sonnet-4-6"
    LLM_TEMPERATURE: float = 0.7
    LLM_MAX_TOKENS: int = 2000

    # ── Hermes Agent bridge (LLM_PROVIDER="hermes") ─────────────────────────
    # Routes every chat turn through the full Hermes Agent (hermes-agent-main)
    # in-process instead of a bare LLM call — gives the avatar every tool
    # Hermes has, including computer_use and (if wired) gui_agent_delegate /
    # UI-TARS. See COMBINED_SETUP.md.
    HERMES_AGENT_PATH: str = ""       # absolute path to your hermes-agent-main/ folder
    HERMES_MODEL: str = ""            # e.g. "anthropic/claude-sonnet-4-6" — empty = Hermes's own default
    HERMES_API_KEY: str = ""          # provider key for Hermes's own model calls
    HERMES_BASE_URL: str = ""         # override provider base_url if needed
    HERMES_PROVIDER: str = ""         # e.g. "openrouter" — empty = Hermes's own configured default
    HERMES_ENABLED_TOOLSETS: str = "" # comma-separated toolset names; empty = Hermes default (all)
    ...
    # For running the GPU-heavy lip-sync step on a separate machine (e.g. a
    # free Kaggle GPU notebook — see musetalk_kaggle_server.ipynb) while this
    # backend itself runs on a weaker local/production machine. Same pattern
    # as the Hermes/UI-TARS remote bridge: this process sends the avatar
    # image + generated audio over HTTP and gets back an mp4.
    MUSETALK_REMOTE_URL: str = ""       # e.g. https://xxxx.trycloudflare.com (from the notebook)
    MUSETALK_REMOTE_API_KEY: str = ""   # must match API_KEY set in the notebook
    MUSETALK_REMOTE_TIMEOUT: int = 120  # seconds
