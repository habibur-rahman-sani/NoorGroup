...
class ConnectionManager:
    ...
    async def _llm_producer(
        ...
        async def _on_tool_event(phase: str, name: str) -> None:
            # Only meaningful for LLM_PROVIDER=hermes — surfaces Hermes tool
            # calls (e.g. gui_agent_delegate driving UI-TARS, or its own
            # computer_use) as a status line while the avatar "thinks".
            label = f"🔧 {name}" if name else "🔧 tool"
            stage = "tool_start" if phase == "start" else "tool_complete"
            await self.send_message(session_id, {"type": "status", "message": label, "stage": stage})

        try:
            with span("llm.stream", **{"history_len": len(messages)}):
                async for token in llm_service.stream_response(
                    messages, system_prompt, session_id=session_id, on_tool_event=_on_tool_event
                ):
