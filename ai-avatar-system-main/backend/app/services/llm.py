...
class LLMService:
    ...
    def __init__(self):
        ...
        if self.provider == "anthropic":
            ...
            # No client to build here — hermes_bridge imports and drives
            # hermes-agent-main's own AIAgent in-process. See
            # app/services/hermes_bridge.py and COMBINED_SETUP.md.
            pass
    ...
    async def generate_response(
        ...
        if self.provider == "hermes":
            return await self._generate_hermes(messages, session_id)
    ...
    async def _generate_hermes(
        ...
        from app.services import hermes_bridge
        ...
        try:
            return await hermes_bridge.generate(sid, last_user)
        except hermes_bridge.HermesBridgeError as e:
            logger.error("hermes_call_failed", extra={"error": str(e)})
    ...
    async def stream_response(
        ...
        if self.provider == "anthropic":
            ...
            async for chunk in self._stream_hermes(messages, session_id, on_tool_event):
                yield chunk
        else:
            raise LLMError(f"Unsupported LLM provider: {self.provider}")
    ...
    async def _stream_hermes(
        ...
        messages: List[Dict[str, str]],
        session_id: Optional[str],
        on_tool_event: Optional[callable],
    ) -> AsyncGenerator[str, None]:
        from app.services import hermes_bridge
        ...
        try:
            async for chunk in hermes_bridge.stream(sid, last_user, on_tool_event=on_tool_event):
                yield chunk
        except hermes_bridge.HermesBridgeError as e:
            logger.error("hermes_stream_failed", extra={"error": str(e)})
            raise LLMUnavailable(str(e)) from e
