...
from app.services.musetalk_remote_client import MuseTalkRemoteError, animate_remote
...
class AvatarAnimator:
    ...
    Supported engines (set AVATAR_ENGINE in .env):
      - musetalk        : MuseTalk V1.5 — persistent LOCAL worker (models loaded once,
                           needs a GPU/model files on this machine)
      - musetalk_remote : same MuseTalk V1.5, but running on a separate machine
                           (e.g. a free Kaggle GPU notebook) — this process sends
                           image+audio over HTTP. See MUSETALK_REMOTE_URL below.
    ...
    async def initialize(self):
        ...
        if self.engine == "musetalk":
            ...
            if not settings.MUSETALK_REMOTE_URL:
                logger.warning(
                    "AVATAR_ENGINE=musetalk_remote but MUSETALK_REMOTE_URL is not "
                    "set. Falling back to simple animation. Paste the URL printed "
                    "by musetalk_kaggle_server.ipynb into .env."
                )
                self.engine = "simple"
            else:
                logger.info(f"Using remote MuseTalk server at {settings.MUSETALK_REMOTE_URL}")
    ...
    async def _animate_musetalk_remote(
        ...
        """Send image+audio to a remote MuseTalk server (e.g. Kaggle) over HTTP."""
        try:
            await animate_remote(
                base_url=settings.MUSETALK_REMOTE_URL,
                api_key=settings.MUSETALK_REMOTE_API_KEY,
                avatar_image_path=avatar_path,
                audio_path=audio_path,
                output_path=output_path,
                timeout=settings.MUSETALK_REMOTE_TIMEOUT,
            )
        except MuseTalkRemoteError as e:
            # Re-raise as RuntimeError so the caller's existing simple-fallback
            # logic (in animate()) handles it the same way as local failures.
